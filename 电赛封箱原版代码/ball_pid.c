...
{
    float pos_err = g_ball_pid.target_cm - pos_cm;
...
{
    g_ball_pid6.pos_kp = 0.9f;
    g_ball_pid6.pos_ki = 0.01f;
    g_ball_pid6.pos_kd = 0.4f;
    g_ball_pid6.pos_integral = 0;
    g_ball_pid6.pos_last_error = 0;
    g_ball_pid6.pos_i_limit = 2.0f;
    g_ball_pid6.pos_target_limit = 5.0f;
    g_ball_pid6.pos_deadband = 0.2f;
    g_ball_pid6.target_cm = 0;
    g_ball_pid6.real_cm = 0;

    g_ball_pid6.vel_kp = 1.0f;
    g_ball_pid6.vel_ki = 0.0f;
    g_ball_pid6.vel_kd = 0.0f;
    g_ball_pid6.target_vel = 0;
    g_ball_pid6.real_vel = 0;
    g_ball_pid6.vel_integral = 0;
    g_ball_pid6.vel_last_error = 0;
    g_ball_pid6.vel_i_limit = 50.0f;
    g_ball_pid6.vel_out_limit = 200.0f;
    g_ball_pid6.vel_deadband = 0.1f;

    g_ball_pid6.output = 0;
    g_ball_pid6.bias   = 0;
}

void ball_pid_set_target6(float cm)
{
    g_ball_pid6.target_cm = cm;
    g_ball_pid6.pos_integral = 0;
    g_ball_pid6.pos_last_error = 0;
}
...
long ball_pid_cascade_run6(float pos_cm, float vel_cm_s)
{
    ...
    float pos_err = g_ball_pid6.target_cm - pos_filt;

    float p = pos_err * g_ball_pid6.pos_kp;

    g_ball_pid6.pos_integral += pos_err * g_ball_pid6.pos_ki * 0.001f;
    if (g_ball_pid6.pos_integral > g_ball_pid6.pos_i_limit)
        g_ball_pid6.pos_integral = g_ball_pid6.pos_i_limit;
    if (g_ball_pid6.pos_integral < -g_ball_pid6.pos_i_limit)
        g_ball_pid6.pos_integral = -g_ball_pid6.pos_i_limit;

    float d = -g_ball_pid6.pos_kd * vel_cm_s;

    float out = p + g_ball_pid6.pos_integral + d;

    if (fabsf(pos_err) < g_ball_pid6.pos_deadband) out = 0;
    if (out > g_ball_pid6.vel_out_limit) out = g_ball_pid6.vel_out_limit;
    if (out < -g_ball_pid6.vel_out_limit) out = -g_ball_pid6.vel_out_limit;

    g_ball_pid6.output = (long)(-out * 200.0f) + g_ball_pid6.bias;
    return g_ball_pid6.output;
}
